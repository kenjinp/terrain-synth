...
const ALPHA_MAP_URL = "https://i.postimg.cc/X7zywCn5/example-alpha.png"
...
export enum CloudAnimationState {
  CHILLING,
  DELETING,
  CREATING,
}
...
export type CloudProps = JSX.IntrinsicElements["group"] & {
  ...
  animationState: CloudAnimationState
}
...
export const Clouds = /* @__PURE__ */ React.forwardRef<Group, CloudsProps>(
  (
    ...
    const shadowInstance = React.useRef<InstancedMesh>(null!)
    ...
    useFrame((state, delta) => {
      ...
      for (index = 0; index < clouds.current.length; index++) {
        ...
        config.shadowMatrix
          .compose(translation, shadowQuat, scale)
          .premultiply(parentMatrix)
      ...
      for (index = 0; index < clouds.current.length; index++) {
        config = clouds.current[index]
        opacities[index] =
          config.ref.current.userData["opacity"] ||
          config.opacity *
        ...
        // Shadow
        shadowInstance.current.setMatrixAt(index, config.shadowMatrix)
        shadowInstance.current.setColorAt(index, config.color)
      }
      ...
      // Update shadow
      shadowInstance.current.geometry.attributes.opacity.needsUpdate = true
      shadowInstance.current.instanceMatrix.needsUpdate = true
      if (shadowInstance.current.instanceColor)
        shadowInstance.current.instanceColor.needsUpdate = true
    })
    ...
    React.useLayoutEffect(() => {
      ...
      setUpdateRange(
        instance.current.geometry.attributes.opacity as BufferAttribute,
    ...
    return (
      <group ref={fref} {...props} name={"banana" + MathUtils.generateUUID()}>
...
>(
  (
    ...
    const [uuid] = React.useState(() => MathUtils.generateUUID())
    ...
    return <group ref={ref} {...props} name={id} />
  },
)
