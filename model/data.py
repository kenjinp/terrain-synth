...
def dataset (downsample_size, number_of_datasamples, elevations_dataset_path, batch_size):
  transform = transforms.v2.Compose([
      ...
      v2.RandomResizedCrop(size=(downsample_size, downsample_size), antialias=True),
      # v2.Resize((DOWNSAMPLE_SIZE, DOWNSAMPLE_SIZE), antialias=True), 
      v2.Normalize(mean=[0.5], std=[0.5]),
  ])

  dataset = datasets.ImageFolder(elevations_dataset_path, transform=transform) # add transformation directly

  subset = list(range(0, number_of_datasamples))
  training_subset = torch.utils.data.Subset(dataset, subset)

  dataloader = torch.utils.data.DataLoader(training_subset, batch_size=batch_size, shuffle=True, num_workers=0)
  return dataloader
